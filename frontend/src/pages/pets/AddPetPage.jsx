...
export default function AddPetPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.ageUnit === 'days' && parsedAge > 0) {
        const d = new Date()
        d.setDate(d.getDate() - parsedAge)
        dataToSubmit.birth_date = d.toISOString().slice(0, 10)
      }

      Object.entries(dataToSubmit).forEach(([k, v]) => fd.append(k, v))
    ...
    <div className="container page-wrapper">
      ...
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', fontWeight: 800 }}>List a Pet</h1>
          <p style={{ color: 'var(--gray-400)', marginTop: 6 }}>Fill in the details to post your pet for adoption or sale.</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button type="submit" className="btn btn-primary btn-lg" disabled={loading} style={{ flex: 1, justifyContent: 'center' }}>
              {loading ? <span className="spinner spinner-sm" /> : <><PlusCircle size={18} /> List My Pet</>}
            </button>
